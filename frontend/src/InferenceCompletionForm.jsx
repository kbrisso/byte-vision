...
// PDF Conversation Export Component
const PDFConversationDocument = ({ chatHistory, documentTitle }) => (
  <Document>
    <Page
      size="A4"
      style={{
...
const InferenceCompletionForm = () => {
  // Chat Service Hook - Make sure to include setHoveredHistoryId
  ...
  const {
    ...
    loadSavedChats,
    hoveredHistoryId,
    setHoveredHistoryId, // This should be included
    clearChatHistory,
    currentMessage,
    isLoadingHistory,
    selectedChatId,
  ...
  // Handle form submission - Fix the undefined globalCliState
  const handleSubmit = useCallback(
    async (event) => {
      ...
      try {
        ...
        // Create the CLI state object from your settings
        const cliState = {
          ...settings?.llamaCli,
          PromptText: currentMessage, // This will be set in generateCompletion
        };
    ...
    [selectedPromptType, currentMessage, sendMessage, settings?.llamaCli],
  );
  ...
  const handleKeyDown = useCallback(
    ...
    [selectedPromptType, currentMessage, isGenerating, handleSubmit],
  );
